...
print("running epsagon's elastalert, with:")
print(f"token: {TOKEN}, app_name: {APP_NAME}, collector_url: {COLLECTOR_URL}")
            ...
            if hash_value != new_rule_hashes[rule_file]:
                ...
                try:
                    ...
                    if "is_enabled" in new_rule and not new_rule["is_enabled"]:
                        elastalert_logger.info(
                            "Rule file %s is now disabled." % (rule_file)
                        )
    ...
    def start(self):
        ...
        while self.running:
            ...
            if self.show_disabled_rules:
                elastalert_logger.info(
                    "Disabled rules are: %s" % (str(self.get_disabled_rules()))
                )
